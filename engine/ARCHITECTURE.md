...
*   **Mechanics**:
    ...
    *   **Narrator status-line fidelity (`narrator-style-fidelity`)**: the prompt contract (default prompt + all four presets + the frontend default) mandates the narrator MUST emit the status line every turn and MUST name the new place in the `Location` field when its narration moves the player (`Location field MUST name the new place`). This is the fix for the stale-location echo that froze the spatial map at 1–3 rooms in natural playtests — the engine commits location from the status line by design, so a narrator that repeats the old location never feeds the room graph a new room. The mandate is pinned by source-text tests in `tests/test_engine_status_parsing.py`. The simple-action narration budget (`computeNarrationBudget`, floor `SIMPLE_ACTION_MIN_TOKENS=200`) guarantees the trailing status line always has output budget — a truncated line parses as nothing and freezes the map — and `sanitizeForHistory` strips truncated `[Status:` fragments as well as complete lines.
    *   **Response-shape exemplar (`system-prompt-response-shape`)**: the prompt contract carries a `RESPONSE SHAPE` section — three tone-neutral complete-turn examples (exploring a new place / talking to someone / a simple action) that each end with the canonical status line as the very last line, with nothing after it and nothing written for the player — so the narrator imitates a well-formed turn instead of inferring the anatomy. The section is defined once as `RESPONSE_SHAPE` in `engine/statusFormat.js` beside `STATUS_FORMAT`; `DEFAULT_SYSTEM_PROMPT` (`engine/index.js`) and the four story presets (`engine/storyPresets.js`) interpolate `${RESPONSE_SHAPE}` (replacing the three inline Zork examples, so the format teaching is not duplicated), and the zero-build frontend default (`web/static/js/app.js`) inlines the identical literal, held honest by source-text pins in `tests/test_engine_status_parsing.py` (marker + "final line" rule across all five producers, plus a constant probe pinning the canonical three-example block and its three-field status lines). No engine-logic change: `parseStatusLine`, reconciliation, the forged-status guard, and the recovery backstop are untouched.
    *   **Stale-status recovery (GH #38, `narrator-style-fidelity`)**: live playtests showed models that won't comply with the mandate (a reasoning model returning empty content, or a default model that chronically repeats a stale `Location`) still freeze the map. `engine/narrationLandmarks.js` is a pure deterministic extractor that recovers a proposed location from the narration's (or the action's) arrival landmarks; `engine/llm.js` fires it when the status line is missing OR the narrator repeats its own previous status line (tracked as `llm.lastStatusLocation`, session-scoped and reset on `newAdventure`/`load`). A changed status always wins; non-arrival prose recovers nothing, so the fallback never fabricates a room from a scene description.
    ...
    *   Before anything is committed, `sanitizeForHistory` (exported from `engine/llm.js`) strips status-line-shaped lines and echoed context blocks. The block strip-set is **derived from the narrator context block registry** (`engine/contextBlocks.js`, `structured-narrator-context`): `buildSystemMessage` composes the system message by iterating the registry (`CONTEXT_BLOCKS`) and emitting each enabled block's header + body, and `sanitizeForHistory` builds its block-strip regex from those same headers at module load — so every injected block (`[CURRENT STATUS]`, `[NARRATOR STYLE]`, `[CURRENT INVENTORY]`, `[ADVENTURE SUMMARY]`, `[WORLD INFO & LORE]`, `[RECALLED MEMORIES]`) is strip-eligible with no second edit anywhere. Each block declares its own `enabled(state, turnContext)` gate (e.g. `[RECALLED MEMORIES]` only when RAG returned rows, `[NARRATOR STYLE]` only once a style is captured) and `build(state, turnContext)` body. The `[CURRENT STATUS]` block renders **byte-identically** to the pre-registry text, pinned by a contract test. The cleaned narration is what reaches `state.history`, the save file, and the memory extraction queue (`bufferTurnPair`). Raw assistant text is retained only in debug/log paths (`llmTracker`/`addDebugLog`) and streaming chunks — never replayed as context.
    *   **Narrator style pin (`narrator-style-fidelity`)**: the narrator is a flexible stylist — it leans into the tone implied by the player's opening and then holds it. On the first player turn (style unpinned), `engine/llm.js` runs the deterministic keyword classifier `detectNarratorStyle` (`engine/narratorStyle.js`) over the title + system prompt + first action, and pins the result into `state.narratorStyle` (persisted as `narrator_style`, additive + null-tolerant). The registry's `[NARRATOR STYLE]` block then renders the adopted style directive into every later turn so the model does not drift; once pinned it is never re-detected (deliberate mid-session restyling is out of scope for v1). `AdventureEngine.setNarratorStyle(label)` is the explicit override.
...
*   **Status-mutation rollback (`make-undo-and-trades-consistent`, D5)**: inventory rollback covers *both* acquisition and status mutation on the undone turn. `upsertInventoryItem` records a `status_turn` on every existing-row status mutation (a guarded `ALTER TABLE` migration adds the column to legacy DBs); `rollbackTurn` (i) deletes rows re-acquired on the undone turn via their `status_turn` even when the original `acquired_turn` predates it (#22), and (ii) restores to `held` (clearing `status_turn`) any pre-existing row whose status was flipped to `traded`/`dropped`/`used`/`equipped` on the undone turn — closing the trade-undo limbo where the sold item was stranded in `traded`. NULL `status_turn` (legacy rows) means "never mutated by a turn" and survives rollback.
...
- **Inventory rollback (D5)**: undo covers status mutations, not just acquisitions. A row re-acquired on the undone turn is removed via its `status_turn` (its original `acquired_turn` predates the turn); a pre-existing row whose status flipped to `traded`/`dropped`/`used`/`equipped` on the undone turn is restored to `held`. An undo never strands an item in `traded`/`dropped` limbo and never leaves a re-acquired item held.
