...
## Purpose
Specifies dynamic viewport scaling (`100dvh`), safe-area inset clearance via CSS token indirection (`--safe-bottom`, `--safe-left`, `--safe-right`, `--tab-bar-h`), virtual keyboard resizing configuration, and non-clipping in-game HUD across mobile and desktop viewports.
...
The application SHALL mediate bottom safe-area insets through CSS custom properties (`--safe-bottom` and `--tab-bar-h`), keeping tab controls and interactive dashboard content elevated above the safe-area inset with at least an 8px clearance floor.
