...
The MCP server process SHALL honor the `SAVE_DIR` declared in `.mcp.json` so that agent-initiated sessions persist under the playtest sandbox directory (`game/playtest/adventures/`) and SHALL NOT fall through to the production directory (`game/adventures/`) during playtest runs.
...
#### Scenario: New playtest session persists to the sandbox
- **WHEN** an AI agent calls `dungeon_init_session` through the MCP server configured with `SAVE_DIR=game/playtest/adventures`
- **THEN** the save file is written under `game/playtest/adventures/` and NOT under the production `game/adventures/` directory
...
The status-line parsing used by `dungeon_send_action` SHALL be the same single shared parser the engine uses. The MCP server SHALL NOT reimplement status parsing in a way that diverges from the engine (including case-sensitivity). State metrics returned by `dungeon_send_action` SHALL be consistent with the engine state that gets persisted.
...
#### Scenario: Send action whose status line uses uppercase STATUS
- **WHEN** the narrator response contains `[STATUS: <Location> | Score: N | Moves: N]` (uppercase) with trailing content after it
- **THEN** `dungeon_send_action` reports the same location/score/moves values the engine committed, and the status line is not persisted as narration
