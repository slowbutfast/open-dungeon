...
The system SHALL expose a mid-session, store-backed view of lore cards and the ability to delete a lore card by ID during an active session, as a recovery path for poisoned or unwanted cards. Deleting a card SHALL remove the row from the structured `lore` store AND the card from `state.cards`, so its trigger words no longer auto-inject on subsequent turns.
