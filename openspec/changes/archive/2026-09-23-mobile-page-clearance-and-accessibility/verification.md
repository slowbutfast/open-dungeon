...
- **Status**: All delta-spec requirements verified passing. `tests/e2e/test_mobile_viewport.py` is **96 passed** (80 pre-existing + 16 new), 0 failed.
- **Date**: 2026-09-23
...
| `mobile-viewport-ergonomics` | `#### Scenario: Restore screen back button clearance` | `test_restore_screen_clearance[*]` (`#restore-screen .btn-back`) — **regression guard**: short content renders trivially clear, so this pins the guard rail rather than the fix | **PASS** |
...
| Wizard panels use `min-height: 100%` (of the bounded scroller's content box), not `100dvh`, so short screens do not force phantom scroll | `test_short_screen_has_no_spurious_scroll` asserts `.app-container` `scrollHeight - clientHeight <= 1` on `#startup-screen` at 375×667; `test_dynamic_viewport_height_declarations` pins the `dvh` cascade to `body`, `.sidebar-panel`, `.modal-content` | `scrollHeight == clientHeight == 667px` → **0px spurious scroll** (was exactly 40px with a `100dvh` panel floor: 707 − 667) | stable |
...
| :--- | :--- | :--- | :--- |
| Keeping `min-height: 100vh; 100dvh;` on wizard panels is harmless inside the bounded scroller (PR #56 "deliberate deviation") | A `100dvh` floor pins each panel to the full 667px viewport inside a padded `.app-container` (content box 627px), forcing `scrollHeight = 707px` vs `clientHeight = 667px` — **exactly 40px of spurious scroll** on every short wizard screen | Panels use `min-height: 100%`; the brittle `#startup-screen` regex in `test_dynamic_viewport_height_declarations` was loosened to assert `dvh` on `body`, `.sidebar-panel`, and `.modal-content`; added `test_short_screen_has_no_spurious_scroll` | Scroll ownership already lived on `.app-container`; only the dvh floor created the phantom 40px. 40px → 0px, eliminating iOS rubber-banding on `#startup-screen` |
| `.app-container` mobile top gutter can shrink to 1rem (PR #56) | Loses the 24px top gutter parity with the 1.5rem side padding on non-notched handsets | `padding-top: max(1.5rem, var(--safe-top))` | Restores symmetric gutters; `--safe-top` still expands on notched devices |
...
### Post-patch `TestMobileScreenBottomClearance` (16/16, includes spurious-scroll guard)
...
### Spurious-scroll probe (375×667, `#startup-screen`, pre/post)

| Metric | `min-height: 100dvh` (PR #56) | `min-height: 100%` (this fix) |
| :--- | :--- | :--- |
| `.app-container` scrollHeight | 707px | 667px |
| `.app-container` clientHeight | 667px | 667px |
| Spurious scroll (scrollHeight − clientHeight) | **40px** | **0px** |

`test_short_screen_has_no_spurious_scroll` asserts the post-fix `delta <= 1` at runtime.
`test_dynamic_viewport_height_declarations` still pins `body { height: 100vh; 100dvh }`,
`.sidebar-panel { max-height: 40vh; 40dvh }`, and `.modal-content { max-height: 90vh; 90dvh }`,
so the `dvh` cascade is verified where it is structurally required.
