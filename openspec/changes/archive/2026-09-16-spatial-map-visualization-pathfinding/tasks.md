...
- [x] 1.6 Write failing e2e for Map Panel Legibility: multi-region `/api/map` fixture via `page.route`; assert canvas within `#tab-map`, current room inside the visible canvas rect, all rooms within scroll bounds (red against `fc95d65`)
...
- [x] 6.6 Size a content layer inside `#map-canvas` instead of the scroll box; move regions/edges/rooms/empty state into it
- [x] 6.7 Width-aware `computeLayout(data, panelWidth)`: cap columns to the panel, wrap region bands into rows, fix clipped region labels; debounced resize re-layout
- [x] 6.8 Scroll the current room into view after render
- [x] 6.9 Trim edge endpoints to the room border so arrowheads are visible
...
- [x] 7.5 Re-run `tests/e2e/test_map_render.py` + full `tests/e2e`; re-run P6 headlessly at desktop + mobile viewports and record the screenshot review
...
- [x] 8.4 Update `web/FRONTEND_ARCHITECTURE.md` renderer paragraph (content-layer sizing, width-aware layout, resize re-layout)
