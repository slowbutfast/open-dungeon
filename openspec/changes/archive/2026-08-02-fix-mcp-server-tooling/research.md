...
## Coordination with `harden-context-history-integrity` (#12)

Verified empirically during implementation (2026-08-02, mock-mode probe): the engine's buffered streaming branch (`engine/llm.js:418`) does **not** commit the status line when the model streams it fragmented (mock emits `cantina.\n[Status:` as one token, producing a buffer `[Status: ... | Score: 5] \n[Status: ... | Score: 5]` the end-anchored regex cannot match). The engine commits `location=West of House, score=0, moves+=1` while the shared MCP parser correctly extracts `location=Cantina, score=5` from the same text.

This is a **transient divergence window**: the MCP side now uses the canonical line-scanning, case-insensitive parser (this change), but the engine's commit path still uses its own end-anchored regex. Until #12 replaces the engine's parsers with the shared `parseStatusLine` exported from `engine/llm.js`, `dungeon_send_action`'s location/score can disagree with `dungeon_inspect_state` on responses the engine fails to parse. `moves` stays consistent (the engine always commits it, and the MCP falls back to engine state when the parser returns null).

Mitigations in this change: `tests/test_shared_status_parser.py` covers parser behavior (uppercase, trailing content, Moveless line, narration stripping) and asserts gameplay.js imports the shared parser; `test_mcp_gameplay.py` asserts moves consistency with committed state. Full location/score end-to-end equality is gated on #12 adopting the same exported parser.
