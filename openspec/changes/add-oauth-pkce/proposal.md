...
Vercel's OAuth token endpoint (`https://api.vercel.com/login/oauth/token`) mandates
Proof Key for Code Exchange (RFC 7636). When initiating the flow without `code_challenge`
and attempting token exchange without `code_verifier`, Vercel rejects the exchange with HTTP 400:

```
OAUTH_CALLBACK_EXCHANGE_FAILED {
  message: 'Vercel token exchange failed (400) {"error":"invalid_grant","error_description":"Invalid code verifier."}',
  redirectUri: 'https://open-dungeon-three.vercel.app/api/auth/callback'
}
```

PKCE is not optional for this provider: Vercel strictly rejects code exchange unless
`code_verifier` matches the `code_challenge` sent at authorization. This change implements
the full PKCE handshake, adds fast-fail error handling for missing verifiers, and pins
computation against RFC 7636 Appendix B.
