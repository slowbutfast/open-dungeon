...
## Vercel Deployment, Auth & Spend Quota (`vercel-deployment-and-auth`)

Five `node:test` suites were added to the unit seam (run via `npm run test:unit`):

| File | What it tests |
|------|---------------|
| `sessionAuth.test.mjs` | `web/auth/session.js` + `web/auth/oauth.js`: HMAC-SHA256 session round-trip `{ sub, email, name, iat, exp, v: 1 }`, tampered payload/signature rejection, wrong-secret rejection, expiration, malformed tokens, the byte-length guard before `crypto.timingSafeEqual` (no throw on length mismatch), cookie attributes (`HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=604800`; state `Max-Age=600`; logout `Max-Age=0`), cryptographically random state, cookie parsing, and the Vercel authorize/token/userinfo wire helpers with an injected `fetchImpl`. |
| `spendLedger.test.mjs` | `web/kvStore.js` (`KvStore`, `SpendLedger`, `acquireLock`/`releaseLock`) and `engine/llmTracker.js` pricing: atomic per-sub increments, the $2.50 lifetime cap (remaining never negative), the $50 global kill-switch, `SET NX` in-flight lock acquire/release + TTL, catalog pricing, multi-operation turn cost, unlisted-model fallback ($0.002/1K), and `llmTracker` turn accumulation via `beginTurn`/`recordUsage`/`getTurnCost`/`endTurn`. Uses the in-memory KV fallback — no Upstash credentials. |
