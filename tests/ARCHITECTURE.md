...
| `vercelEntry.test.mjs` | `api/index.js` exports a dispatching Express app (`/api/ping` → 200); `validateProductionConfig` fails closed when secrets are missing, `MOCK_LLM=1`, or `LLM_BACKEND !== 'openrouter'`, and accepts a complete env; child-process boot checks confirm `api/index.js` exits non-zero on a bad Vercel env and zero on a complete one. |
